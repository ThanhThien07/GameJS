...
def update_exact_cover():
    ...
    # 1. Update Cover Page Paragraphs (P0 to P23) exactly
    def set_p_text(p, text, bold=True, size_pt=14):
    ...
    # P7: Title
    set_p_text(doc.paragraphs[7], "PHÂN TÍCH THIẾT KẾ VÀ XÂY DỰNG GAME SIÊU CLICKER TAM HỢP\n(TAP TAP CLICKER MULTIPLAYER)", bold=True, size_pt=18)
    
    # P8: Semester
    set_p_text(doc.paragraphs[8], "HỌC KỲ 3, NĂM HỌC 2025 - 2026", bold=True, size_pt=14)
    
    # P10: Group Name
    set_p_text(doc.paragraphs[10], "NHÓM DECIBEL", bold=True, size_pt=16)

    # P15: NSVTH 1
    set_p_text(doc.paragraphs[15], "NSVTH: NGUYỄN HOÀNG HƯNG  (MSSV: 501250384)", bold=True, size_pt=13)
    
    # P16: NSVTH 2
    set_p_text(doc.paragraphs[16], "\t\t   HOÀNG THỊ DIỄM LY", bold=True, size_pt=13)
    
    # P17: Clear 3rd author
    set_p_text(doc.paragraphs[17], "", bold=False, size_pt=13)

    # P18: Subject
    set_p_text(doc.paragraphs[18], "MÔN : JAVASCRIPTS", bold=True, size_pt=13)
    
    # P19: Class
    set_p_text(doc.paragraphs[19], "LỚP: CD25CT6", bold=True, size_pt=13)

    # P20: Lecturer
    set_p_text(doc.paragraphs[20], "GVHD: ThS. Trương Châu Long", bold=True, size_pt=13)

    # P23: Date
    set_p_text(doc.paragraphs[23], "Tp. Hồ Chí Minh, tháng 07 năm 2026", bold=True, size_pt=12)

    # 2. Build the full Game Report content after Cover Page (from P51 onwards)
    # Remove old body paragraphs after cover page (from index 24 onwards)
    # We will keep paragraphs 0-23 as the exact cover page!
    
    # Remove paragraphs after index 23
    p_elements = [p._element for p in doc.paragraphs[24:]]
    for pe in p_elements:
        pe.getparent().remove(pe)

    # Also remove any old tables
    ...
    # ==========================================
    # 1. GIỚI THIỆU PHẦN MỀM GAME CLICKER
    # ==========================================
    ...
    # ==========================================
    # 4. ĐẶC TẢ HỆ THỐNG CƠ SỞ DỮ LIỆU & ARCHITECTURE
    # ==========================================
    ...
    # ==========================================
    # 5. CÔNG NGHỆ SỬ DỤNG IN PROJECT
    # ==========================================
    ...
    print(f"Exact cover and content saved to {output_path} successfully!")
