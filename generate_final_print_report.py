...
def build_final_print_report():
    ...
    # Print-ready paragraph helper (Times New Roman, Justified, 1.2 Line Spacing)
    def add_body_p(text="", align=WD_ALIGN_PARAGRAPH.JUSTIFY, bold=False, italic=False, size=13, color=(30, 41, 59), space_before=2, space_after=5, line_spacing=1.2):
        p = doc.add_paragraph()
        p.alignment = align
        p.paragraph_format.space_before = Pt(space_before)
        p.paragraph_format.space_after = Pt(space_after)
        p.paragraph_format.line_spacing = line_spacing
        p.paragraph_format.first_line_indent = Inches(0.3) # Standard paragraph indent for print
        if text:
            run = p.add_run(text)
    ...
    for title, pg in toc_items:
        ...
        is_main = title.startswith("CHƯƠNG") or title in ["LỜI CẢM ƠN", "MỤC LỤC", "MỞ ĐẦU", "KẾT LUẬN VÀ HƯỚNG PHÁT TRIỂN"]
        
        run_t = p_toc.add_run(title)
        run_t.font.name = 'Times New Roman'
        run_t.font.size = Pt(12)
        run_t.font.bold = is_main
        run_t.font.color.rgb = RGBColor(15, 23, 42) if is_main else RGBColor(51, 65, 85)

        # Tab dots filler
        dots_count = max(4, 90 - len(title) * 2)
        run_dots = p_toc.add_run(" " + "." * (dots_count // 2) + " ")
    ...
    print(f"Print-ready report saved to {output_path} successfully!")

if __name__ == '__main__':
    build_final_print_report()
